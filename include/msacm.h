...
/* Windows CE COREDLL driver-management exports (signatures from the
 * Windows CE SDK msacm.h). */
#ifdef _WIN32_WCE
MMRESULT WINAPI acmDriverDetails(HACMDRIVERID hadid, LPACMDRIVERDETAILS padd, DWORD fdwDetails);
...
/* Windows CE ACM additions: COREDLL exports the UNICODE-only filter,
   filter-tag and choose surfaces plus the format surface in their W
   shape (interface facts from the Windows CE SDK msacm.h).  The filter
   family has no desktop A/W counterpart in this header, so the CE
   structures are defined here under their SDK names; the format family
   reuses the LPACMFORMATDETAILSW layout above. */
#ifdef _WIN32_WCE
#pragma pack(push, 1)
...
typedef UINT (CALLBACK *ACMFILTERTAGENUMCB)(HACMDRIVERID, LPACMFILTERTAGDETAILS, DWORD, DWORD);
typedef BOOL (CALLBACK *ACMFILTERENUMCB)(HACMDRIVERID, LPACMFILTERDETAILS, DWORD, DWORD);
typedef UINT (CALLBACK *ACMFILTERCHOOSEHOOKPROC)(HWND, UINT, WPARAM, LPARAM);
...
typedef UINT (CALLBACK *ACMFORMATCHOOSEHOOKPROC)(HWND, UINT, WPARAM, LPARAM);
...
#ifndef acmFormatEnum
MMRESULT WINAPI acmFormatEnum(HACMDRIVER had, LPACMFORMATDETAILSW pafd, ACMFORMATENUMCBW fnCallback, DWORD dwInstance, DWORD fdwEnum);
#endif
MMRESULT WINAPI acmFormatChoose(LPACMFORMATCHOOSE pafmtc);
MMRESULT WINAPI acmFilterTagDetails(HACMDRIVER had, LPACMFILTERTAGDETAILS paftd, DWORD fdwDetails);
MMRESULT WINAPI acmFilterTagEnum(HACMDRIVER had, LPACMFILTERTAGDETAILS paftd, ACMFILTERTAGENUMCB fnCallback, DWORD dwInstance, DWORD fdwEnum);
MMRESULT WINAPI acmFilterDetails(HACMDRIVER had, LPACMFILTERDETAILS pafd, DWORD fdwDetails);
MMRESULT WINAPI acmFilterEnum(HACMDRIVER had, LPACMFILTERDETAILS pafd, ACMFILTERENUMCB fnCallback, DWORD dwInstance, DWORD fdwEnum);
MMRESULT WINAPI acmFilterChoose(LPACMFILTERCHOOSE pafltrc);
#endif /* _WIN32_WCE */
